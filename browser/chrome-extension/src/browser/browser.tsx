...
const BrowserUI = () => {
    ...
    const [browserRefresh, setBrowserRefresh] = useState<number>(0);
    ...
    chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
        if (request.type === "PING_JSON_VIEWER") {
            ...
            setBrowserRefresh(r => r + 1);
        }
    });

    return (
        <ContextProviders isBrowser jsonViewerOptions={jsonViewerOptions} browserRefresh={browserRefresh}>
            <BrowserContent />
        </ContextProviders>
    );
};
