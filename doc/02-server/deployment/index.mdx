...
| `hypr-mediator` | Distributes operations over Hyperswarm and reports direct peer connections | internal `4003` |
| `search-server` | Builds a searchable read model from Gatekeeper's index export | `4002` |
| `explorer` | Displays DIDs, events, credentials, receipts, metrics, and live Hyperswarm peers | `4000` |
...
Gatekeeper is the core service. Keymaster and the mediators submit operations to it. Search Server consumes Gatekeeper's ordered index export and proxies the Hyperswarm mediator's local peer status. Explorer reads Search Server. IPFS is optional and controlled by a Compose profile.
...
| `KC_KEYMASTER_BIND_ADDRESS` | Host interface for Keymaster's Compose-published port. Keep the default `127.0.0.1` unless access is protected by an appropriate private network or authenticated proxy |
...
Explorer's Network tab shows the direct peers visible to the local Hyperswarm mediator. It reports the status as unavailable when `hypr-mediator` is not running.
...
Keymaster controls the server wallet and must remain private. Compose binds its host port to `127.0.0.1` by default. Database and blockchain RPC ports must also remain private. For a read-only public resolver, expose only the required Gatekeeper read endpoints, preferably through a firewall and TLS reverse proxy.
